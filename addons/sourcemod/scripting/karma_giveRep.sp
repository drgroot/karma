...
public Action giveRep( client, args ){
	...
	if( GetCmdArgs() == 0 && strcmp(command,"sm_rep") == 0){
		...
		"SELECT TIMESTAMPDIFF(MINUTE, a.lastRep, now() ),%d, '%s','%s', TIMESTAMPDIFF(minute,b.rep_date,now()), b.steamID FROM players a LEFT JOIN players_reputation b ON a.`steamID`=b.`from` WHERE a.steamID = '%s'"
